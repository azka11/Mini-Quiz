...
import { getSpecificAdmin, editProfile } from "../modules/fetch/admin";
...
function AdminSettingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSelectedImage(`http://localhost:3000/${admin.image}`)
      } catch (error) {
  ...
  const hideAdminSetting = () => setAdminSetting(false);
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
      await editProfile(data.admin_id, formData);
            ...
            <div className="flex justify-end m-5">
              <h1
                className={`text-black font-normal text-2xl ${
                  !adminSetting ? "font-semibold" : "hover:font-medium"
                } 
                    cursor-pointer`}
                onClick={hideAdminSetting}
              >
                General
              </h1>
            </div>
